...
/* The following details are relevant for Spectrum-2 and Spectrum-3.
 * In Spectrum-4, the chunks are build differently. See more details below.
 */
...
/* The hash function which is used for Spectrum-2 and Spectrum-3
 * is different from Spectrum-4. See below.
 */
 ...
 * The result is comination of the two calculations -
 ...
 */
static const u16 mlxsw_sp_acl_bf_crc10_tab[256] = {
 ...
 */
static const u16 mlxsw_sp_acl_bf_crc6_tab[256] = {
0x0000, 0x002d, 0x0037, 0x001a, 0x0003, 0x002e, 0x0034, 0x0019,
0x0006, 0x002b, 0x0031, 0x001c, 0x0005, 0x0028, 0x0032, 0x001f,
0x000c, 0x0021, 0x003b, 0x0016, 0x000f, 0x0022, 0x0038, 0x0015,
0x000a, 0x0027, 0x003d, 0x0010, 0x0009, 0x0024, 0x003e, 0x0013,
0x0018, 0x0035, 0x002f, 0x0002, 0x001b, 0x0036, 0x002c, 0x0001,
0x001e, 0x0033, 0x0029, 0x0004, 0x001d, 0x0030, 0x002a, 0x0007,
0x0014, 0x0039, 0x0023, 0x000e, 0x0017, 0x003a, 0x0020, 0x000d,
0x0012, 0x003f, 0x0025, 0x0008, 0x0011, 0x003c, 0x0026, 0x000b,
0x0030, 0x001d, 0x0007, 0x002a, 0x0033, 0x001e, 0x0004, 0x0029,
0x0036, 0x001b, 0x0001, 0x002c, 0x0035, 0x0018, 0x0002, 0x002f,
0x003c, 0x0011, 0x000b, 0x0026, 0x003f, 0x0012, 0x0008, 0x0025,
0x003a, 0x0017, 0x000d, 0x0020, 0x0039, 0x0014, 0x000e, 0x0023,
0x0028, 0x0005, 0x001f, 0x0032, 0x002b, 0x0006, 0x001c, 0x0031,
0x002e, 0x0003, 0x0019, 0x0034, 0x002d, 0x0000, 0x001a, 0x0037,
0x0024, 0x0009, 0x0013, 0x003e, 0x0027, 0x000a, 0x0010, 0x003d,
0x0022, 0x000f, 0x0015, 0x0038, 0x0021, 0x000c, 0x0016, 0x003b,
0x000d, 0x0020, 0x003a, 0x0017, 0x000e, 0x0023, 0x0039, 0x0014,
0x000b, 0x0026, 0x003c, 0x0011, 0x0008, 0x0025, 0x003f, 0x0012,
0x0001, 0x002c, 0x0036, 0x001b, 0x0002, 0x002f, 0x0035, 0x0018,
0x0007, 0x002a, 0x0030, 0x001d, 0x0004, 0x0029, 0x0033, 0x001e,
0x0015, 0x0038, 0x0022, 0x000f, 0x0016, 0x003b, 0x0021, 0x000c,
0x0013, 0x003e, 0x0024, 0x0009, 0x0010, 0x003d, 0x0027, 0x000a,
0x0019, 0x0034, 0x002e, 0x0003, 0x001a, 0x0037, 0x002d, 0x0000,
0x001f, 0x0032, 0x0028, 0x0005, 0x001c, 0x0031, 0x002b, 0x0006,
0x003d, 0x0010, 0x000a, 0x0027, 0x003e, 0x0013, 0x0009, 0x0024,
0x003b, 0x0016, 0x000c, 0x0021, 0x0038, 0x0015, 0x000f, 0x0022,
0x0031, 0x001c, 0x0006, 0x002b, 0x0032, 0x001f, 0x0005, 0x0028,
0x0037, 0x001a, 0x0000, 0x002d, 0x0034, 0x0019, 0x0003, 0x002e,
0x0025, 0x0008, 0x0012, 0x003f, 0x0026, 0x000b, 0x0011, 0x003c,
0x0023, 0x000e, 0x0014, 0x0039, 0x0020, 0x000d, 0x0017, 0x003a,
0x0029, 0x0004, 0x001e, 0x0033, 0x002a, 0x0007, 0x001d, 0x0030,
0x002f, 0x0002, 0x0018, 0x0035, 0x002c, 0x0001, 0x001b, 0x0036,
};
...
static u16 mlxsw_sp_acl_bf_crc10_byte(u16 crc, u8 c)
{
	u8 index = ((crc >> 2) ^ c) & 0xff;

	return ((crc << 8) ^ mlxsw_sp_acl_bf_crc10_tab[index]) & 0x3ff;
}

static u16 mlxsw_sp_acl_bf_crc6_byte(u16 crc, u8 c)
{
	u8 index = (crc ^ c) & 0xff;

	return ((crc << 6) ^ (mlxsw_sp_acl_bf_crc6_tab[index] << 2)) & 0xfc;
}

static u16 mlxsw_sp4_acl_bf_crc(const u8 *buffer, size_t len)
{
	const u8 *buffer_row = buffer, *buffer_col = buffer;
	size_t len_row = len, len_col = len;
	u16 crc_row = 0, crc_col = 0;

	while (len_row--)
		crc_row = mlxsw_sp_acl_bf_crc10_byte(crc_row, *buffer_row++);

	while (len_col--)
		crc_col = mlxsw_sp_acl_bf_crc6_byte(crc_col, *buffer_col++);
...
static void right_shift_array(char *arr, u8 shift_bits, u8 len)
{
...
static void mlxsw_sp4_bf_key_shift_chunks(u8 chunk_count, char *output)
{
	 ...
	 * and in Spectrum-4 there is no padding, there is a need to shift some
	 * chunks 2 bits right.
	 */
	switch (chunk_count) {
	case 2:
		 ...
		 */
		right_shift_array(output, 2, MLXSW_SP4_BLOOM_KEY_CHUNK_BYTES);
		break;
	case 3:
		/* The chunks are copied as follow:
		 * +-------------+-----------------+-----------------+
		...
		right_shift_array(output, 4, MLXSW_SP4_BLOOM_KEY_CHUNK_BYTES);
		break;
	default:
