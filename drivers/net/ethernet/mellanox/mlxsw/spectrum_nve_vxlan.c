...
static int
mlxsw_sp2_nve_ethertype_to_push_set(struct mlxsw_sp *mlxsw_sp, u16 ethertype)
{
	...
	mlxsw_reg_spvid_local_port_set(spvid_pl,
				       MLXSW_REG_SPVID_TUNNEL_PORT_NVE);
	err = mlxsw_reg_query(mlxsw_sp->core, MLXSW_REG(spvid), spvid_pl);
	if (err)
		return err;
			       ...
			       const struct mlxsw_sp_nve_config *config)
{
	...
	mlxsw_reg_spvtr_pack(spvtr_pl, true, MLXSW_REG_SPVTR_TUNNEL_PORT_NVE,
	...
	err = mlxsw_sp2_nve_ethertype_to_push_set(mlxsw_sp, config->ethertype);
	if (err)
		goto err_ethertype_to_push_set;

	return 0;

err_ethertype_to_push_set:
	mlxsw_reg_spvtr_pack(spvtr_pl, true, MLXSW_REG_SPVTR_TUNNEL_PORT_NVE,
...
static void mlxsw_sp2_nve_vxlan_config_clear(struct mlxsw_sp *mlxsw_sp)
{
	...
	mlxsw_sp2_nve_ethertype_to_push_set(mlxsw_sp, ETH_P_8021Q);
	mlxsw_reg_spvtr_pack(spvtr_pl, true, MLXSW_REG_SPVTR_TUNNEL_PORT_NVE,
			     MLXSW_REG_SPVTR_IPVID_MODE_IEEE_COMPLIANT_PVID);
	mlxsw_reg_write(mlxsw_sp->core, MLXSW_REG(spvtr), spvtr_pl);
	mlxsw_reg_tngcr_pack(tngcr_pl, MLXSW_REG_TNGCR_TYPE_VXLAN, false, 0);
