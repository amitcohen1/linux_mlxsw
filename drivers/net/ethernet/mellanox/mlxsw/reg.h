 ...
 * 0 - do not update VID.
 * 1 - set VID.
 * For Spectrum-2 when set_vid=0 and smpe_valid=1 then the
 * smpe will modify the vid.
 * Access: RW
 *
 ...
 * When ubridge=1, it is reserved when set_vid=0.
 * Access: RW
 */
 ...
 * 1 - router sub-port flooding table, for this fid the flooding
 * is per router-sub-port local_port.
 * Must not be set for a fid which is not a router-sub-port and must be set
 * prior to enabling the relevant RIF.
 * Access: RW
 *
 * Note: Reserved when ubridge = 0.
 */
 ...
 * ------------------------------------------
 * The switch multicast port to egress VID maps {egress_port, SMPE} -> {VID}.
 */
 ...
 * SMPE is valid.
 * When not valid, the egress VID will not be modified by the
 * SMPE table.
 ...
 * When ubridge = 0: Only FIDs from the vFID range are supported.
 * When ubridge = 1: This is the egress FID for router to bridge.
 * Access: RW
 */
...
/* reg_ritr_sp_if_efid
 * Egress filtering ID.
 * Used to connect the eRIF to a bridge if eRIF-ACL has
 * modified the DMAC or the VID.
 * Access: RW
 *
 * Note: Reserved when ubridge = 0.
 */
