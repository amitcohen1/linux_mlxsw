...
ns1_create()
{
	...
	in_ns ns1 \
	      ns_init_common v2 192.0.2.34 192.0.2.50 vx100 1000 192.0.2.33

	in_ns ns1 bridge vlan add vid 100 dev vx100 pvid untagged
	in_ns ns1 simple_if_init w2
	in_ns ns1 vlan_create w2 10 vw2 192.0.2.3/28
}
...
ns2_create()
{
	...
	in_ns ns2 \
	      ns_init_common v4 192.0.2.50 192.0.2.34 vx200 2000 192.0.2.49

	in_ns ns2 simple_if_init w2 192.0.2.4/28
}
...
ping_ipv4()
{
	...
	ping_test $h2 192.0.2.4 ": local->remote 2"
}
