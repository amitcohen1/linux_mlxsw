...
linkmodes_get_policy[ETHTOOL_A_LINKMODES_MAX + 1] = {
	...
	[ETHTOOL_A_LINKMODES_LANES]		= { .type = NLA_REJECT },
};
				  ...
				  struct genl_info *info)
{
	...
	if (!(dev->ethtool_ops->capabilities & ETHTOOL_CAP_FORCE_LINK_LANES_SUPPORTED))
	...
	*mod = false;
	...
	if (req_lanes) {
		...
		if (!lsettings->autoneg &&
		    !(dev->ethtool_ops->capabilities & ETHTOOL_CAP_FORCE_LINK_LANES_SUPPORTED)) {
			NL_SET_ERR_MSG_ATTR(info->extack,
					    tb[ETHTOOL_A_LINKMODES_LANES],
